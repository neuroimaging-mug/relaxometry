 ...
 */
#include "CLevenbergMarquardtFletcherProcessor.hpp"
...
#include "CLevenbergMarquardtFletcher.cpp"

CLevenbergMarquardtFletcherProcessor::CLevenbergMarquardtFletcherProcessor() {
}
...
vector<ProcessorData*> CLevenbergMarquardtFletcherExecuteCore(Model* model, ProcessorContext* processorContext) {
	...
	//Process
	NTOUTPUT* output = CProcessLevenbergMarquardtFletcher<NTINPUT, NTOUTPUT, NTCALC>(
...
vector<ProcessorData*> CLevenbergMarquardtFletcherExecuteCoreCaller(Model* model, ProcessorContext* processorContext) {
	...
	if(processorContext->getUseDoublePrecision()) {
		if(outputDataType == INT16)
			return CLevenbergMarquardtFletcherExecuteCore<NTINPUT, short, double>(model, processorContext);
		else if(outputDataType == FLOAT32)
			return CLevenbergMarquardtFletcherExecuteCore<NTINPUT, float, double>(model, processorContext);
		else if(outputDataType == UINT16)
			return CLevenbergMarquardtFletcherExecuteCore<NTINPUT, unsigned short, double>(model, processorContext);
		else
			throw runtime_error("Not supported output data type: " + to_string(outputDataType));
	} else {
		if(outputDataType == INT16)
			return CLevenbergMarquardtFletcherExecuteCore<NTINPUT, short, float>(model, processorContext);
		else if(outputDataType == FLOAT32)
			return CLevenbergMarquardtFletcherExecuteCore<NTINPUT, float, float>(model, processorContext);
		else if(outputDataType == UINT16)
			return CLevenbergMarquardtFletcherExecuteCore<NTINPUT, unsigned short, float>(model, processorContext);
		else
			throw runtime_error("Not supported output data type: " + to_string(outputDataType));
	}
}

vector<ProcessorData*> CLevenbergMarquardtFletcherProcessor::Execute(Model* model,
	ProcessorContext* processorContext) {
	ProcessorDataDataType dataType = model->getEchoesData()->getDataType();
	if(dataType == INT16)
		return CLevenbergMarquardtFletcherExecuteCoreCaller<short>(model, processorContext);
	else if(dataType == FLOAT32)
		return CLevenbergMarquardtFletcherExecuteCoreCaller<float>(model, processorContext);
	else if(dataType == UINT16)
		return CLevenbergMarquardtFletcherExecuteCoreCaller<unsigned short>(model, processorContext);
	else
		throw runtime_error("Not supported data type: " + to_string(dataType));
}

bool CLevenbergMarquardtFletcherProcessor::IsCUDAProcessor() {
	return false;
}