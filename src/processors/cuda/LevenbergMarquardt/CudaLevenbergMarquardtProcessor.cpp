 ...
 */
#include "CudaLevenbergMarquardtProcessor.hpp"

#include "../../../includes/StringExtensions.h"
#include "../../ProcessorService.hpp"
#include "../../../models/Model.hpp"
#include "CudaLevenbergMarquardt.hpp"
...
CudaLevenbergMarquardtProcessor::CudaLevenbergMarquardtProcessor() {
}
...
vector<ProcessorData*> CudaLevenbergMarquardtExecuteCore(Model* model, ProcessorContext* processorContext) {
ProcessorData* processorData = model->getEchoesData();
	...
	//Process
	NTOUTPUT* output = CudaProcessLevenbergMarquardt<NTINPUT, NTOUTPUT, NTCALC>(
...
vector<ProcessorData*> CudaLevenbergMarquardtExecuteCoreCaller(Model* model, ProcessorContext* processorContext) {
	ProcessorDataDataType outputDataType = model->getOutputDataType();
	if(outputDataType == INT16)
		return CudaLevenbergMarquardtExecuteCore<NTINPUT, short, float>(model, processorContext);
	else if(outputDataType == FLOAT32)
		return CudaLevenbergMarquardtExecuteCore<NTINPUT, float, float>(model, processorContext);
	else if(outputDataType == UINT16)
		return CudaLevenbergMarquardtExecuteCore<NTINPUT, unsigned short, float>(model, processorContext);
	else
		throw runtime_error("Not supported output data type: " + to_string(outputDataType));
}

vector<ProcessorData*> CudaLevenbergMarquardtProcessor::Execute(Model* model,
	ProcessorContext* processorContext) {
	ProcessorDataDataType dataType = model->getEchoesData()->getDataType();
	if(dataType == INT16)
		return CudaLevenbergMarquardtExecuteCoreCaller<short>(model, processorContext);
	else if(dataType == FLOAT32)
		return CudaLevenbergMarquardtExecuteCoreCaller<float>(model, processorContext);
	else if(dataType == UINT16)
		return CudaLevenbergMarquardtExecuteCoreCaller<unsigned short>(model, processorContext);
	else
		throw runtime_error("Not supported data type: " + to_string(dataType));
}

bool CudaLevenbergMarquardtProcessor::IsCUDAProcessor() {
	return true;
}