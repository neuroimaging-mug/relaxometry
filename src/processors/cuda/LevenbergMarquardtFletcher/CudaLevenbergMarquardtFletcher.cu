...
#include "CudaCalculateAdjustedRsquareValue.cuh"
...
#include "CudaLevenbergMarquardtFletcherCore.cu"

template<typename NTOUTPUT, typename NTCALC>
__global__ void CudaLevenbergMarquardtFletcher(
  ...
  stepVariables.beta =  stepVariables.alpha + parametersCount * parametersCount;
	...
	CudaLevenbergMarquardtFletcherCore(
		modelFunctionID,
...
#ifdef USE_SHARED_MEMORY
		CudaLevenbergMarquardtFletcher<<<blockCount, threadsPerBlock, sharedMemoryPerThreadBytes>>>(
			...
			EPSILON_1, EPSILON_2);
#else
		CudaLevenbergMarquardtFletcher<<<blockCount, threadsPerBlock>>>(
			startModelFunctionID, modelFunctionID,
			residualWeightingFunctionID, alphaWeightingFunctionID,
			parametersCount,
			restructeredColumnsCount, restructeredRowsCount, echoesCount,
...
template
short* CudaProcessLevenbergMarquardtFletcher(
	short startModelFunctionID, short modelFunctionID,
	short residualWeightingFunctionID, short alphaWeightingFunctionID,
	int parametersCount,
	int startIndex, int endIndex, int columnCount, int rowCount, 
...
template
float* CudaProcessLevenbergMarquardtFletcher(
	short startModelFunctionID, short modelFunctionID,
	short residualWeightingFunctionID, short alphaWeightingFunctionID,
	int parametersCount,
	int startIndex, int endIndex, int columnCount, int rowCount, 
...
template
unsigned short* CudaProcessLevenbergMarquardtFletcher(
	short startModelFunctionID, short modelFunctionID,
	short residualWeightingFunctionID, short alphaWeightingFunctionID,
	int parametersCount,
	int startIndex, int endIndex, int columnCount, int rowCount, 
